...
import { useGetAllProductsQuery } from "@/services/data/product.data";
...
const Menu: React.FC = () => {
  ...
  const handleSelectCategory = React.useCallback(
    (cat: Category) => {
      const categoriesWithProducts = data?.data?.categories;
      const finalProducts = categoriesWithProducts?.filter((category) => {
        if (category.id === cat.id) {
          return category;
        }
      });

      console.log("PRODUCTS ARRAY 24", finalProducts);
  ...
  React.useEffect(() => {
    ...
    const finalCategories = finalData?.categories?.map((category) => {
      return category;
    });
    console.log("FINAL CATEGORIES", finalCategories);
    setCategories(finalCategories);
    setSelectedCategory(finalCategories[0]);
    handleSelectCategory(finalCategories[0]);
  }, [data, handleSelectCategory]);
            ...
            <div className=" flex gap-2">
              <div className=" relative flex items-center ">
                ...
                <Image
                  src={SearchIcon}
                  alt="icon"
                  className="absolute inset-y-0 left-0 w-5 mt-3 ml-4 pointer-events-none text-gray-400"
                />
              </div>
          ...
          <div className="grid grid-cols-6 gap-8 mt-4 ">
            ...
            <div className="bg-cardBg hidden lg:col-span-4 lg:block rounded-2xl p-6 border border-primary">
              <div className=" flex justify-between items-center mb-4  ">
                <h1 className=" text-darkGrey text-2xl font-normal">
                  All Dishes
                </h1>
